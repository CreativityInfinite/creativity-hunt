    ...
    <div className="relative">
      ...
      <section className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16 min-h-screen">
        ...
        <div className="mb-6 sm:mb-8 lg:mb-10 text-center">
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-semibold tracking-tight">{messages?.headings?.explore || '探索全部分类'}</h1>
          <p className="mt-2 text-sm sm:text-base text-muted-foreground">{messages?.hero?.subtitle || '发现优质 AI 工具与平台，按分类和用途快速定位'}</p>
        </div>
        ...
        <div className="grid grid-cols-12 gap-4 sm:gap-6 lg:gap-8 max-w-7xl mx-auto">
          ...
          <aside className="col-span-12 sm:col-span-3 lg:col-span-3">
            <div className="sticky top-16 rounded-2xl border border-border/50 bg-card/60 backdrop-blur p-3 sm:p-4">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left px-3 py-3 rounded-xl transition-all duration-200 flex items-center gap-3 mb-2 ${
                      isActive ? 'bg-primary/10 text-primary border border-primary/20 shadow-sm' : 'hover:bg-accent/40 border border-transparent'
                    }`}
          ...
          <main className="col-span-12 sm:col-span-9 lg:col-span-9">
            {/* 搜索框 */}
            <div className="mb-4">
              <div className="relative rounded-2xl border border-border/50 bg-card/60 backdrop-blur p-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full rounded-xl border border-border/50 bg-background/50 pl-10 pr-4 py-2.5 text-sm placeholder:text-muted-foreground focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all duration-200"
                  />
                </div>
              </div>
            </div>
            ...
            {/* 标签式子分类选择器 */}
            <div className="mb-6">
              <div className="flex flex-wrap gap-2 p-4 rounded-2xl border border-border/50 bg-card/60 backdrop-blur">
                {isClient &&
                  (['全部', ...subs] as string[]).map((label) => {
                    const isActive = activeSub === label;
                    const displayName = label === '全部' ? '全部' : getSubcategoryDisplayName(activeCategory, label as any, messages);
                    const count = label === '全部' ? currentTools.length : currentTools.filter((t) => t.subcategory === label).length;
                    return (
                      <button
                        key={label}
                        ...
                        className={`px-3 py-1.5 rounded-full text-sm font-medium transition-all duration-200 border flex items-center gap-1.5 ${
                          isActive
                            ...
                            : 'bg-background/50 text-muted-foreground border-border/50 hover:bg-accent hover:text-accent-foreground hover:border-accent'
                        }`}
                      >
                ...
                {!isClient && (
                  <button className="px-3 py-1.5 rounded-full text-sm font-medium bg-primary text-primary-foreground border border-primary shadow-sm flex items-center gap-1.5">
                    <span>全部</span>
                    <span className="text-xs px-1.5 py-0.5 rounded-full bg-primary-foreground/20 text-primary-foreground">{currentTools.length}</span>
                  </button>
                )}
              </div>
            </div>
            ...
            <div className="mt-6">
              ...
              <div className="mb-4 flex items-center gap-2 text-sm text-muted-foreground">
                ...
                <span>·</span>
                <span>{activeSub === '全部' ? '全部' : isClient ? getSubcategoryDisplayName(activeCategory, activeSub as any, messages) : activeSub}</span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 lg:gap-6">
                {(() => {
                  ...
                  return displayList.map((tool) => <ToolCard key={tool.name} tool={tool} />);
                })()}
                {(() => {
                  // 计算过滤后的结果数量
                  let displayList = currentTools.filter((t) => (activeSub === '全部' ? true : t.subcategory === activeSub));
                    ...
                    displayList.length === 0 && (
                      <div className="col-span-full text-center text-sm text-muted-foreground py-12">
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <div>{searchQuery.trim() ? '未找到匹配的工具，尝试调整搜索关键词' : '暂无内容，切换其他二级分类试试'}</div>
                        </div>
                      </div>
                    )
                  );
